...
from database.db_config import DB_PATH, get_connection


def create_tables() -> None:
    """Create the local sqlite database schema and seed defaults."""
    ...
    try:
        ...
        cur.execute(
            "INSERT OR IGNORE INTO University (university_id, university_code, university_name) "
            "VALUES (1, 'SCH001', 'Default University')"
        )
        cur.execute(
            "INSERT OR IGNORE INTO departments (department_id, department_code, department_name, university_id) "
        ...
        if active_count == 0:
            cur.execute(
                "INSERT OR IGNORE INTO semesters (semester_id, semester_name, start_date, status) "
